...
pub fn SvgSinkOrSource(
    ...
    let text_width = 50.0;
    let text_x = if text_left {
        x + width * 0.5 - text_width
    } else {
        x + width * 0.5
    };

    let class = if text_left {
        "truncate font-medium text-end"
    } else {
...
pub fn SvgLine(x1: f64, y1: f64, x2: f64, y2: f64) -> impl IntoView {
    view! { <line x1=x1 y1=y1 x2=x2 y2=y2 stroke-width="2" stroke="black" stroke-linecap="round" /> }
}
