...
pub fn sync_spatial_regions_with_systems(
    ...
    mut environment_query: Query<
        (&EnvironmentRegion, &mut Transform),
        (Without<System>, Without<BoundaryRegion>),
    >,
    ...
    for (boundary_region, mut boundary_transform) in boundary_query.iter_mut() {
        ...
        if let Ok(system_transform) = system_query.get(boundary_region.system_entity) {
            ...
            if system_transform.translation.x.is_finite()
                && system_transform.translation.y.is_finite()
            {
                ...
                trace!(
                    "Synced boundary region to system {:?} at {:?}",
                    boundary_region.system_entity,
                    boundary_transform.translation
                );
            } else {
            ...
            trace!(
                "Skipping boundary sync for system {:?} - entity invalid (probably during cleanup)",
                boundary_region.system_entity
            );
        }
    }
    ...
    for (environment_region, mut environment_transform) in environment_query.iter_mut() {
        ...
        if let Ok(system_transform) = system_query.get(environment_region.system_entity) {
                ...
                && system_transform.translation.y.is_finite()
            {
                ...
                trace!(
                    "Synced environment region to system {:?} at {:?}",
                    environment_region.system_entity,
                    environment_transform.translation
                );
            } else {
...
pub fn apply_zoom_to_spatial_regions(
    mut boundary_query: Query<
        (&BoundaryRegion, &mut Transform),
        (With<BoundaryRegion>, Without<System>),
    >,
    mut environment_query: Query<
        (&EnvironmentRegion, &mut Transform),
        (
            With<EnvironmentRegion>,
            Without<System>,
            Without<BoundaryRegion>,
        ),
    >,
    system_query: Query<
        &Transform,
    ...
    for (boundary_region, mut boundary_transform) in boundary_query.iter_mut() {
        ...
        if let Ok(system_transform) = system_query.get(boundary_region.system_entity) {
                ...
                && zoom_scale.is_finite()
            {
                ...
                trace!(
                    "Applied zoom {:.2} to boundary region of system {:?}",
                    zoom_scale,
                    boundary_region.system_entity
                );
            } else {
    ...
    for (environment_region, mut environment_transform) in environment_query.iter_mut() {
        ...
        if let Ok(system_transform) = system_query.get(environment_region.system_entity) {
                ...
                && zoom_scale.is_finite()
            {
                ...
                trace!(
                    "Applied zoom {:.2} to environment region of system {:?}",
                    zoom_scale,
                    environment_region.system_entity
                );
            } else {
