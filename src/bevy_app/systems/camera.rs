...
pub fn pan_camera_with_mouse(
    ...
    for event in motion_events.read() {
        camera_query.single_mut().unwrap().translation +=
...
pub fn pan_camera_with_mouse_wheel(
    ...
    for event in scroll_events.read() {
        ...
        camera_query.single_mut().unwrap().translation += match event.unit {
...
pub fn reset_camera_position(mut camera_query: Query<&mut Transform, With<Camera>>) {
    camera_query.single_mut().unwrap().translation = Vec3::new(0.0, 0.0, 1000.);
}
