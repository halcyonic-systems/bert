...
use crate::bevy_app::data_model::{WorldModel, HasInfo};
...
fn count_components_by_level(world_model: &WorldModel, breakdown: &mut ComplexityBreakdown) {
    ...
    let external_count = world_model.environment.sources.len() + world_model.environment.sinks.len();
...
mod tests {
    use super::*;
    use crate::bevy_app::data_model::{WorldModel, CURRENT_FILE_VERSION, Environment, Info, Id, IdType};
    ...
    fn test_cell_json_complexity() {
        ...
        if let Ok(cell_json_bytes) = fs::read("assets/models/cell.json") {
            ...
            if let Ok(world_model) = serde_json::from_slice::<WorldModel>(&cell_json_bytes) {
                ...
                assert!(complexity.total_complexity > 1.0, "Should have meaningful complexity");
